...
function getAllBooks()
{
	...
	if (navigator.userAgent.match(/iPhone/i)||navigator.userAgent.match(/iPad/i)) {
        ...
        iframe.setAttribute("src", "anreader:afd:myaction:afd:getAllBooks");
...
function creatBookShelf(id, name, author, coverimage, bookpath)
{
    ...
    setLayoutLocation(document.body.scrollHeight);
    $(window).resize(function () { setLayoutLocation(document.body.scrollHeight);});
}
/**set dispaly add and delete button on book shelf**/
function setDisplayButton(isDisplay)
{   
    displayAddButton = (isDisplay == 1) ? true: false;
    if (!displayAddButton){
        $("#import").hide();
        $("#edit").hide();
    }
...
{
    if (navigator.userAgent.match(/Android/i)) {
        displayAddButton =  Android.getShelfEdit();
    }
    if (navigator.userAgent.match(/iPhone/i)||navigator.userAgent.match(/iPad/i)) {
        window.location = 'anreader:afd:myaction:afd:getShelfEdit';
        //return window[callback](objcResult);
    }
    return true;
}
